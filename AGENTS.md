...
- Keep Custom Builder Preview drag/drop wired to guide-level `guideFlowEdits.itemOrder`. Keep it separate from Guide Card preview drag/drop, which is card-only customization storage.
- Use the leader-note resolver helpers when displaying, counting, editing, or deleting Custom Builder leader notes. Do not read only `customGuidance` because starter/template and Quick Builder notes may originate from guide `steps`.
